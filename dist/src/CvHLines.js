...
var CvHLines = /** @class */ (function () {
    ...
    CvHLines.prototype.paint = function () {
        ...
        for (var i = 0; i < this.nTria; i++) {
            ...
            var b = vScr[tri.iB];
            var c = vScr[tri.iC];
            // Determinar el color según el rango de los vértices
            // Si TODOS los vértices del triángulo están en la Luna (1-386)
            if (tri.iA >= 1 && tri.iA <= 386 &&
                tri.iB >= 1 && tri.iB <= 386 &&
                tri.iC >= 1 && tri.iC <= 386) {
                this.g.fillStyle = 'rgb(128, 194, 238)';
            }
            // Si TODOS los vértices del triángulo están en el Satélite (387-3926)
            else if (tri.iA >= 387 && tri.iA <= 3926 &&
                tri.iB >= 387 && tri.iB <= 3926 &&
                tri.iC >= 387 && tri.iC <= 3926) {
                this.g.fillStyle = 'rgb(169, 75, 25)';
            }
            // Si es mixto, usa el color original (verde)
            else {
                this.g.fillStyle = 'rgb(220, 46, 199)';
            }
            this.g.beginPath();
